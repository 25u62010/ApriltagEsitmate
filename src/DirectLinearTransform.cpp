...
bool DirectLinearTransform::CalT(TagsPos& detections, cv::Mat& T) {
    if (detections.size() < 8) {
        cout << "The number of detections must larger than 8." << endl;
        return false;
    }
    cv::RNG rng;
    ...
    for (int i = 0; i < mnIterations; i++) {
        ...
        unordered_map<int, double> scalars1;
        for (int j = 0; j < 8; j++) {
...
cv::Mat DirectLinearTransform::toCvMat(const g2o::SE3Quat& SE3) {
    ...
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++) cvMat.at<double>(i, j) = eigMat(i, j);
    return cvMat.clone();
}
